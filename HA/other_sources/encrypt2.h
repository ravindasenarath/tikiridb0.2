...
void encrypt_with_ha_pub_key(char * data){
...
/*****************  Encrypt SK2 by HA  *********************/
char* encrypt_sk2 (char* data, const char* usrPubKey){
	//testing purpose
	int dataSize = 37 ; // 128 for NO PADDING, __ANY SIZE UNDER 128 B__ for RSA_PKCS1_PADDING
  	unsigned char *str = makeAlphaString( dataSize ) ;
  	strcpy(str, data);
	...
	RSA *pubKey = loadPUBLICKeyFromString(usrPubKey);
